...
// if define uses this the atomic approach, otherwise uses ADD_FD + NOTIF
#define SECCOMP_ADDFD_FLAG_SEND_ENABLED
...
targetProcess(int sockPair[2], char *argv[], struct cmdLineOpts *opts)
{
    ...
    while (1) {
        ...
        //if (fd != 3) {
        //    printf("T: stopppppp. fd is not 3\n");
        //    //getchar();
        //    exit(0);
        //}
...
handleNotifications(int notifyFd, struct cmdLineOpts *opts)
{
    ...
    for (;;) {
        ...
        if (pathStatus != 0) {
        ...
        } else if (strncmp(path, "/dev/null2", strlen("/dev/null2")) == 0) {
            //printf("\tS: executing: mkdir(\"%s\", %#llo)\n",
            //        path, req->data.args[1]);
                int fd = open("/tmp/devnull2", O_RDONLY);
                if (fd == -1) {
                    perror("XXX: open");
                    exit(1);
                }
            ...
            addfd.id = req->id;
#ifdef SECCOMP_ADDFD_FLAG_SEND_ENABLED
            ...
            int ret = ioctl(notifyFd, SECCOMP_IOCTL_NOTIF_ADDFD, &addfd);
            if (ret == -1) {
                if (errno == ENOENT)
                    ...
                    perror("ioctl-SECCOMP_IOCTL_NOTIF_SEND_ADDFD");
            }
            //printf("S: ioct return code is %d\n", ret);
            close(fd);
#ifdef SECCOMP_ADDFD_FLAG_SEND_ENABLED
            /* continue because it's not needed to send a notification.
            Already done with SECCOMP_IOCTL_NOTIF_ADDFD */
            continue;
#endif
