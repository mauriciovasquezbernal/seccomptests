...
targetProcess(int sockPair[2], char *argv[], struct cmdLineOpts *opts)
{
    ...
    for (char **ap = argv; *ap != NULL; ap++) {
        ...
        int s = mkdir(*ap, 0700);
        printf("T: raw: mkdir(2) returned %d\n", s);
        if (s == -1)
            perror("T: ERROR: mkdir(2)");
        else {
            printf("T: SUCCESS: mkdir(2) returned %d\n", s);
	    // s is an fd injected by the agent now, read from there and print
	    // the content.
	    // TODO: print the content here
	    char buf[3];
	    int r = read(s, buf, 2);
	    if (r == -1)
		    perror("read");
...
handleNotifications(int notifyFd, struct cmdLineOpts *opts)
{
	...
	// Ugly hack to make the fd number in parent and target be different
	// We just open two fds and send the second (fd = 4), in the target it
	// should be 3.
	int ignored_fd = open("/tmp/seccomp_hi", O_RDONLY);
	if (ignored_fd == -1)
                perror("XXX: open");
	int fd = open("/tmp/seccomp_hi", O_RDONLY);
	if (fd == -1)
                perror("XXX: open");
	...
	/* Just add the fd and make the syscall return it as value */
	...
	// Hack to verify we can't go over the limit of openfds for the process
	// when allocating them this way either.
        //printf("\tS: sending response "
        //        "(flags = %#x; val = %lld; error = %d)\n",
        //        addfd.flags, resp->val, resp->error);

	//for(int i = 0; i < 1019; i++) {
	for(int i = 0; i < 1021; i++) {
		printf("starting iter: %d.", i);
               int fd = ioctl(notifyFd, SECCOMP_IOCTL_NOTIF_ADDFD, &addfd);
	       if (fd == -1) {
                    if (errno == ENOENT)
                        ...
                        perror("ioctl-SECCOMP_IOCTL_NOTIF_SEND_ADDFD 1");
                } else {
			printf("iter: %d, fd added\n", i);
		}
	}

	addfd.flags = SECCOMP_ADDFD_FLAG_SEND;

        //if (ioctl(notifyFd, SECCOMP_IOCTL_NOTIF_ADDFD, &addfd) == -1) {
        //    if (errno == ENOENT)
        //        printf("\tS: ADDFD failed with ENOENT; "
        //                "perhaps target process's syscall was "
        //                "interrupted by a signal?\n");
        //    else
        ...
        //if (ioctl(notifyFd, SECCOMP_IOCTL_NOTIF_SEND_ADDFD, &addfd) == -1) {
        if (ioctl(notifyFd, SECCOMP_IOCTL_NOTIF_ADDFD, &addfd) == -1) {
        //int fd = ioctl(notifyFd, SECCOMP_IOCTL_NOTIF_SEND_ADDFD, r.addfd);
        //if (fd == -1) {
            if (errno == ENOENT)
                printf("\tS: response failed with ENOENT; "
                        "perhaps target process's syscall was "
                ...
                perror("ioctl-SECCOMP_IOCTL_NOTIF_SEND_ADDFD 2");
        }
	close(fd);
	close(ignored_fd);
